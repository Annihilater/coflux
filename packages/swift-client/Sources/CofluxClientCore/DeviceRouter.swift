...
struct DeviceRouteError: LocalizedError {
...
final class DeviceRouter {
    ...
    private final class Route {
        ...
        var heartbeatUnsupported = false
    ...
    private let controlGraceDuration: Duration
    private var controlGeneration = 0
    private var controlGraceTask: Task<Void, Never>?
    private var graceChannels: [String: Channel] = [:]
    ...
    init(
        ...
        self.controlGraceDuration = controlGraceDuration
    }
    ...
    private func invalidateOnlineControl() {
        ...
        for route in routes.values {
            ...
            session.promotionTask?.cancel(); session.promotionTask = nil
            session.attempt?.cancel(); session.attempt = nil
            session.recoveryTask?.cancel(); session.recoveryTask = nil
            session.token += 1
            closeLane(route, route.elevatedLane, reason: "中心在线授权已失效")
        }
    }
    ...
    func setControlDisconnected() {
        guard controlOnline else { return }
        controlOnline = false
        controlGeneration += 1
        let generation = controlGeneration
        invalidateOnlineControl()
        clearControlGrace()
        for route in routes.values {
            if let channel = route.sessionLane.active, !channel.local {
                graceChannels[channel.channelID] = channel
            }
        }
        controlGraceTask = Task { [weak self] in
            guard let self else { return }
            do { try await Task.sleep(for: self.controlGraceDuration) } catch { return }
            guard !self.controlOnline, self.controlGeneration == generation else { return }
            self.controlGraceTask = nil
            self.graceChannels.removeAll()
            self.closeRemoteSessions()
        }
    }

    /// false 是明确撤权：即使已经处于网络断线宽限，也必须立即关闭远端 session。
    ...
    func setAccountID(_ value: String?) {
        guard accountID != value else { return }
        reset()
        accountID = value
    }
    ...
    func hasSessionControl(daemonID: String, sessionID: String) -> Bool {
        guard let route = routes[daemonID], let session = route.sessions[sessionID],
              let channel = route.sessionLane.active, usable(channel) else { return false }
        return session.desired && !session.detached && session.holderEpoch != nil
    ...
    func listDirectory(daemonID: String, workspaceID: String, path: String, browseHome: Bool) async throws -> Coflux_V1_FsListed {
        var value = Coflux_V1_DeviceFsList()
        value.requestID = UUID().uuidString
        value.workspaceID = workspaceID
        value.path = path
        value.browseHome = browseHome
        let result = try await request(routeFor(daemonID), lane: .elevated, requestID: value.requestID, payload: .fsList(value))
        guard case .fsListed(let listed) = result else { throw DeviceRouteError("目录响应类型错误") }
        return listed
    }

    func readFile(daemonID: String, workspaceID: String, path: String) async throws -> Coflux_V1_FsReadResult {
        var value = Coflux_V1_DeviceFsRead()
        value.requestID = UUID().uuidString
        value.workspaceID = workspaceID
        value.path = path
        let result = try await request(routeFor(daemonID), lane: .elevated, requestID: value.requestID, payload: .fsRead(value))
        guard case .fsReadResult(let read) = result else { throw DeviceRouteError("文件响应类型错误") }
        return read
    }

    func execute(daemonID: String, workspaceID: String, command: String, args: [String]) async throws -> Coflux_V1_ExecResult {
        var value = Coflux_V1_DeviceExecRun()
        value.requestID = UUID().uuidString
        value.operationID = UUID().uuidString
        value.workspaceID = workspaceID
        value.command = command
        value.args = args
        value.timeoutMs = 20_000
        let result = try await request(routeFor(daemonID), lane: .elevated, requestID: value.requestID,
                                       payload: .execRun(value), timeout: .seconds(25))
        guard case .execResult(let execution) = result else { throw DeviceRouteError("命令响应类型错误") }
        return execution
    }
    ...
    /// 允许 lane 空闲释放；有终端需求的现存连接继续探活，不依赖面板可见性。
    func retainMeasure(daemonID: String) -> @MainActor () -> Void {
        let route = routeFor(daemonID)
    ...
    private func stopHeartbeat(_ route: Route) {
        route.pingTask?.cancel(); route.pingTask = nil
    ...
    private func startPingLoop(_ route: Route) {
        guard route.pingTask == nil, route.sessionLane.active != nil, !route.heartbeatUnsupported else { return }
    ...
    private func pingOnce(_ route: Route) {
        guard !route.heartbeatUnsupported, route.pendingPing == nil,
              routes[route.daemonID] === route,
              let channel = route.sessionLane.active, usable(channel) else { return }
        var ping = Coflux_V1_DevicePing()
        ping.requestID = UUID().uuidString
        route.pendingPing = (ping.requestID, channel.generation, now())
        sendOn(route, channel, .ping(ping))
        route.heartbeatTimeoutTask = Task { [weak self] in
            guard let self else { return }
            do { try await Task.sleep(for: self.heartbeatTimeout) } catch { return }
            guard self.routes[route.daemonID] === route, route.sessionLane.active === channel,
                  route.pendingPing?.id == ping.requestID else { return }
            route.pendingPing = nil
            route.heartbeatTimeoutTask = nil
            route.rttMs = nil
            route.heartbeatMisses += 1
            if route.heartbeatMisses >= 2 {
                self.loseChannel(route, channel, reason: "心跳连续无响应")
            } else {
                self.callbacks.onDeviceTransport(route.daemonID, route.relayHost, nil, route.mode, route.detail)
                self.pingOnce(route)
            }
        }
    }
    ...
    private func ensureLane(_ route: Route, _ lane: Lane) async throws -> Channel {
        ...
        let attempt = Task<Channel, any Error> { [weak self] in
            ...
            let channel = try await self.openPreferredChannel(route, lane)
            ...
            guard self.usable(channel), !Task.isCancelled else { self.closeChannel(channel); throw CancellationError() }
            self.activate(route, lane, channel)
            return channel
        }
        lane.attempt = attempt
        defer { if lane.token == token { lane.attempt = nil } }
        do { return try await attempt.value }
        catch {
            if lane.kind == .session, lane.token == token, routes[route.daemonID] === route, !(error is CancellationError) {
                publishDiagnostic(route, mode: "offline", detail: error.localizedDescription)
            }
            throw error
        }
    }

    private func publishDiagnostic(_ route: Route, mode: String, detail: String) {
        route.mode = mode; route.detail = detail; route.rttMs = nil; route.relayHost = nil
        callbacks.onDeviceTransport(route.daemonID, nil, nil, mode, detail)
    }

    private func usable(_ channel: Channel) -> Bool {
        guard !channel.closed else { return false }
        if !channel.local {
            return controlOnline || (channel.lane == .session && graceChannels[channel.channelID] === channel)
        }
        if channel.lane == .session { return channel.scopes.isSuperset(of: [.sessionRead, .sessionControl]) }
        return controlOnline && channel.scopes.isSuperset(of: [.rpc, .lifecycle]) && (channel.leaseExpiresAt ?? 0) > now() + 2000
    }

    private func completeTransportControl(_ id: String, payload: Coflux_V1_ServerToClient.OneOf_Payload) {
        guard let waiter = transportWaiters.removeValue(forKey: id) else { return }
        waiter.timeout.cancel(); waiter.continuation.resume(returning: payload)
    }
    private func rejectTransportControl() {
        let pending = transportWaiters; transportWaiters.removeAll()
        for waiter in pending.values {
            waiter.timeout.cancel(); waiter.continuation.resume(throwing: DeviceRouteError("设备传输授权请求已取消"))
        }
    }
    private func requestTransportControl(_ payload: Coflux_V1_ClientToServer.OneOf_Payload) async throws -> Coflux_V1_ServerToClient.OneOf_Payload {
        ...
        return try await withTaskCancellationHandler {
            ...
            return try await withCheckedThrowingContinuation { continuation in
                ...
                transportWaiters[id] = TransportControlWaiter(continuation: continuation, timeout: timeout)
                callbacks.sendControl(payload)
            }
        } onCancel: {
            Task { @MainActor [weak self] in
                guard let waiter = self?.transportWaiters.removeValue(forKey: id) else { return }
                waiter.timeout.cancel(); waiter.continuation.resume(throwing: CancellationError())
            }
        }
    }
    ...
    private func openDirectChannel(_ route: Route, _ lane: Lane) async throws -> Channel {
        ...
        let generation = nextGeneration(route.daemonID)
        let opened = try await localProvider.open(daemonID: route.daemonID, accountID: accountID, clientInstanceID: clientInstanceID,
            generation: generation, elevated: lane.kind == .elevated, authorize: { [weak self] payload in
                guard let self else { throw CancellationError() }
                return try await self.requestTransportControl(payload)
            })
        let channel = Channel(channelID: opened.channelID, generation: generation, lane: lane.kind, connection: opened.connection,
                              relayHost: nil, local: true, scopes: Set(opened.scopes), leaseExpiresAt: opened.leaseExpiresAt)
        guard !Task.isCancelled, usable(channel) else { closeChannel(channel); throw DeviceRouteError("本机连接授权不足或已过期") }
        return channel
    }
    ...
    private func closeLane(_ route: Route, _ lane: Lane, reason: String) {
        ...
        if lane.kind == .session {
            ...
            if route.relayHost != nil || route.rttMs != nil || route.mode != nil {
                route.relayHost = nil
                route.rttMs = nil
                route.mode = nil
                route.detail = ""
                callbacks.onDeviceTransport(route.daemonID, nil, nil, nil, "")
            }
        }
    }
    ...
    private func sendOn(_ route: Route, _ channel: Channel, _ payload: Coflux_V1_DeviceEnvelope.OneOf_Payload) {
        guard usable(channel) else { loseChannel(route, channel, reason: "连接授权已失效"); return }
        guard let frame = encodeFrame(channel, payload) else { return }
        let connection = channel.connection
        let previous = channel.sendChain
        channel.sendChain = Task { [weak self] in
            await previous?.value
            guard let self else { return }
            guard self.usable(channel) else { self.loseChannel(route, channel, reason: "连接授权已失效"); return }
            do {
    ...
    private func request(
        ...
        return try await withTaskCancellationHandler {
            ...
            Task { @MainActor [weak self] in
                guard let self, let route = self.routes[daemonID],
                      let pending = route.pendingRequests.removeValue(forKey: requestID) else { return }
    ...
    private func handleDevicePayload(_ route: Route, _ channel: Channel, _ payload: Coflux_V1_DeviceEnvelope.OneOf_Payload) {
        switch payload {
        case .pong(let pong):
            guard let pending = route.pendingPing, pending.id == pong.requestID,
                  pending.generation == channel.generation, route.sessionLane.active === channel else { return }
            route.rttMs = max(0, now() - pending.startedAt)
            route.pendingPing = nil
            route.heartbeatTimeoutTask?.cancel(); route.heartbeatTimeoutTask = nil
            route.heartbeatMisses = 0
            callbacks.onDeviceTransport(route.daemonID, route.relayHost, route.rttMs, route.mode, route.detail)
    ...
    private func handleDeviceError(_ route: Route, _ channel: Channel, requestID: String?, code: String, message: String) -> Bool {
        if let pending = route.pendingPing, route.sessionLane.active === channel,
           ...
           (requestID == nil || requestID == pending.id) {
            stopHeartbeat(route)
            route.heartbeatUnsupported = true
            route.rttMs = nil
            callbacks.onDeviceTransport(route.daemonID, route.relayHost, nil, route.mode, route.detail)
            return true
        }
        if let requestID {
            for session in route.sessions.values {
    ...
    private static func requestID(of payload: Coflux_V1_DeviceEnvelope.OneOf_Payload) -> String? {
        ...
        case .fsList(let value): return value.requestID
        case .fsRead(let value): return value.requestID
        case .execRun(let value): return value.requestID
        case .projectValidate(let value): return value.requestID
        case .worktreeAdd(let value): return value.requestID
        case .worktreeRemove(let value): return value.requestID
        default: return nil
        }
    }

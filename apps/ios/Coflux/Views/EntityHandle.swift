...
/// coflux 实体句柄（plan 20260914-entity-handles）：`coflux:<kind>:<短 id>`，
/// 短 id 恒取实体 UUID 的前 8 位十六进制小写。句柄是给用户复制、粘给 agent 用的
/// 一串"看得出是 coflux 的、看得出是什么东西的"标识——任何接受 id 的接口同样接受它。
///
/// 句柄由各客户端本地拼装（protobuf 契约不带 `ref`，plan 已定案），所以这里是 iOS 侧
...
enum EntityHandle {
    ...
    /// id 为空时返回 nil：宁可不给这一项菜单，也不发出一个拼不全的句柄。
...
extension View {
    /// 长按复制该实体的句柄。句柄不作为常驻界面文本出现（plan 定案），
        ...
        contextMenu {
            if let handle = EntityHandle.make(kind, id: id) {
                Button {
                    ...
                    Label("复制句柄", systemImage: "doc.on.doc")
                }
            }
        }
    }
